...
func NewTrendsRepo(db *pgxpool.Pool) repository.TrendsRepo { return &pgTrendsRepo{db} }

func (r *pgTrendsRepo) ListSnapshots(ctx context.Context, userID string) ([]trends.NetWorthSnapshot, error) {
	rows, err := r.db.Query(ctx,
		`SELECT id, user_id, snapshot_date, assets_value, cash_position, net_worth, note
		 FROM net_worth_snapshots WHERE user_id = $1 ORDER BY snapshot_date ASC`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var out []trends.NetWorthSnapshot
	for rows.Next() {
		var s trends.NetWorthSnapshot
		var d time.Time
	...
	if out == nil {
		out = []trends.NetWorthSnapshot{}
	}
	return out, rows.Err()
}

func (r *pgTrendsRepo) UpsertSnapshot(ctx context.Context, s trends.NetWorthSnapshot) (trends.NetWorthSnapshot, error) {
...
func (r *pgTrendsRepo) UpsertBenchmark(ctx context.Context, b trends.BenchmarkData) error {
	_, err := r.db.Exec(ctx,
...
func (r *pgTrendsRepo) ListBenchmarks(ctx context.Context, sources []string, from, to string) ([]trends.BenchmarkData, error) {
	var q string
	var args []interface{}
	if len(sources) > 0 {
	...
	defer rows.Close()
	var out []trends.BenchmarkData
	for rows.Next() {
		var b trends.BenchmarkData
		var d time.Time
	...
	if out == nil {
		out = []trends.BenchmarkData{}
	}
	return out, rows.Err()
}

func (r *pgTrendsRepo) UpsertBankRate(ctx context.Context, b trends.BankRate) error {
	today := b.FetchedDate
	if today == "" {
...
func (r *pgTrendsRepo) LatestBankRates(ctx context.Context) ([]trends.BankRate, error) {
	rows, err := r.db.Query(ctx,
	...
	bankMap := map[string]*trends.BankRate{}
	for rows.Next() {
		var src string
		var val float64
		var d time.Time
		...
		if bankMap[bank] == nil {
			bankMap[bank] = &trends.BankRate{Bank: bank, FetchedDate: dateStr}
		}
		switch kind {
		case "saving":
	...
	var out []trends.BankRate
	seen := map[string]bool{}
	for _, bank := range order {
		if br, ok := bankMap[bank]; ok {
			out = append(out, *br)
			seen[bank] = true
		}
	}
	for bank, br := range bankMap {
		if !seen[bank] {
			out = append(out, *br)
		}
	}
...
func (r *pgTrendsRepo) ListNews(ctx context.Context, limit int) ([]trends.NewsItem, error) {
	rows, err := r.db.Query(ctx,
	...
	defer rows.Close()
	var out []trends.NewsItem
	for rows.Next() {
		var n trends.NewsItem
		var pub time.Time
...
func (r *pgTrendsRepo) UpsertNews(ctx context.Context, items []trends.NewsItem) error {
	for _, n := range items {
		_, err := r.db.Exec(ctx,
